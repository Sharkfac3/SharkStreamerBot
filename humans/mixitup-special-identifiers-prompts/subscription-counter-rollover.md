# Prompt: Subscription Counter Rollover

You are working in `SharkStreamerBot`. Backfill Mix It Up special identifiers for this Streamer.bot action script.

Target script: `Actions/Twitch Core Integrations/subscription-counter-rollover.cs`
Matching README: `Actions/Twitch Core Integrations/README.md`
...
Update `Actions/Twitch Core Integrations/subscription-counter-rollover.cs` so its Mix It Up payload sends populated `SpecialIdentifiers` instead of an empty object.

Recommended identifiers:

- `subtype` — `counterrollover`
- `subrollover` — `rollover` arg as string
- `subrollovercount` — `rolloverCount` arg as string
- `subcounter` — `subCounter` arg as string

## Notes

This is a counter event, not a user event. Do not add `subuser` unless Streamer.bot docs prove a user arg exists for this trigger.
...
- `Actions/Twitch Core Integrations/README.md` documents the final payload shape and identifier list.

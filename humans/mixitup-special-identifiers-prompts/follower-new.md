...
Target script: `Actions/Twitch Core Integrations/follower-new.cs`
...
Update `Actions/Twitch Core Integrations/follower-new.cs` so its Mix It Up payload sends populated `SpecialIdentifiers` instead of an empty object.

Recommended identifiers:

- `followuser` — `user` arg
- `followuserid` — `userId` arg
- `followtype` — `new`

## Notes

This script currently has a placeholder command ID and empty special identifiers. Add helper methods consistent with the subscription scripts.

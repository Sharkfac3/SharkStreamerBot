...
Update `Actions/Twitch Hype Train/hype-train-end.cs` so its Mix It Up payload sends populated `SpecialIdentifiers` instead of an empty object.
...
End events do not document `expiresAt`, `duration`, or all-time-high fields in the current README. Do not add undocumented fields unless verified.

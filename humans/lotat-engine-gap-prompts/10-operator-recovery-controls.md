...
Your focused task:
Resolve the ambiguity around operator recovery controls.

Specifically investigate:
- which recovery controls are required in v1
- from which runtime stages each control is allowed
- what side effects each control should have on timers, vote state, branch history, chaos, and messaging
- how to keep recovery deterministic and safe for live stream use
- what should count as a normal resolution versus an override path

Questions you should be ready to ask me may include:
- Which controls are must-have before first implementation?
- Should force-close resolve normally or use a special override result?
- Should manual advance append history as if a normal choice won?
- What operator visibility is needed before using a recovery action?

Do not add recovery-contract changes until I answer your questions.

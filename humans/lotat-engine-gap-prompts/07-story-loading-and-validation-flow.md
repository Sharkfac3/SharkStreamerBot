...
Your focused task:
Resolve the ambiguity around story loading and preflight validation.

Specifically investigate:
- what file the engine should actually load in v1
- whether `Creative/WorldBuilding/Storylines/loaded/current-story.json` is the only runtime source of truth
- when validation happens relative to session start and join open
- what happens if the file is missing, malformed, or inconsistent with the contract
- what operator handoff flow is expected between `ready/`, `loaded/`, and runtime use

Questions you should be ready to ask me may include:
- Should the engine ever scan `ready/` directly?
- Should session start fail before `join_open` if validation fails?
- How much operator tooling is assumed versus manual file loading?
- Do we need a separate validation-only step before runtime loading?

Do not change pipeline docs or file-path assumptions until I answer your questions.

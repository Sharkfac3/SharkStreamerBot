...
Your focused task:
Resolve the ambiguity around `chaos.on_success` and `chaos.on_failure`.

Specifically investigate:
- how the engine is supposed to determine whether a resolved choice counts as a success or failure
- whether the current schema is sufficient to support that determination
- whether the outcome classification belongs on the choice, the node, runtime-only logic, or somewhere else
- whether a v1 contract should simplify chaos handling instead of expanding schema immediately
- migration risks to existing story JSON if we change this

Questions you should be ready to ask me may include:
- Do we want success/failure to be authored explicitly or inferred?
- Is partial success a needed runtime concept before implementation?
- Should chaos be per-node only, per-choice, or both?
- Is a non-breaking fallback acceptable for existing stories?

Do not make any schema changes until I answer your questions.

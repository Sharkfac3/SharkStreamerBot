...
Your focused task:
Resolve the ambiguity around `supported_mechanics` semantics.

Specifically investigate:
- whether `supported_mechanics` is informational metadata, a validation contract, or an engine feature gate
- what the engine should do if a story declares a mechanic as true or false
- whether unsupported mechanics should block runtime loading
- how this field should coordinate with runtime behavior and story validation
- whether v1 should treat some flags as decorative and others as enforced

Questions you should be ready to ask me may include:
- Should `landing_party: true` hard-fail a story today?
- If `dice_hooks: false`, should authored dice hooks be a validation error?
- Are these flags promises from the story, capabilities requested from the engine, or both?
- Do we expect the engine to ignore extra metadata or reject it?

Do not change the schema meaning or validation behavior until I answer your questions.

...
public class CPHInline
{
    ...
    private const string SCENE_SECTION_LABEL = "Housekeeping";
    ...
    /*
     * Purpose:
     * - Switches OBS to the Housekeeping scene for the current stream mode.
     *
    ...
    public bool Execute()
    {
        ...
        {
            CPH.LogWarn("[Voice Commands: Scene Housekeeping] Could not resolve a target scene.");
            return true;
        }

        if (!TrySetObsScene(targetScene))
        {
            CPH.LogWarn($"[Voice Commands: Scene Housekeeping] Failed to switch OBS scene to '{targetScene}'.");
        }
    ...
    private string ResolveTargetScene(string mode)
    {
        switch (mode)
        {
            ...
            default:
                CPH.LogWarn($"[Voice Commands: Scene Housekeeping] Unknown stream_mode '{mode}'. Falling back to workspace scene.");
                return BuildSceneName(SCENE_PREFIX_WORKSPACE);
        }
    }
    ...
    private bool TryInvokeStringArg(string methodName, string arg)
    {
        ...
        {
            CPH.LogWarn($"[Voice Commands: Scene Housekeeping] OBS method '{methodName}' failed: {ex.Message}");
            return false;
        }
    }
}

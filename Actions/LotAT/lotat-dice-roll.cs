...
public class CPHInline
{
    ...
    /*
     * Purpose:
     * - Shared !roll intake for LotAT dice windows.
     * - Accepts rolls only during dice_open, allows any chatter to roll,
     *   surfaces every 1-100 result to chat, and resolves immediately on the first success.
     *
     * Expected trigger/input:
     * - Streamer.bot chat command trigger for !roll.
     * - Reads sender from `user` / `userName`.
     *
     * Required runtime variables:
     * - Reads: active/stage/window guards, current node id, and dice success threshold.
     * - Writes: dice resolved state, timer stop on success, and normal decision-window globals.
     *
     * Key outputs/side effects:
     * - Ignores !roll outside dice_open.
     * - Does not consult the joined roster.
     * - Emits every roll result to chat in v1.
     * - On first roll >= threshold, emits authored success_text and opens the normal decision window.
     *
     * Operator notes:
     * - Route only !roll to this action.
     * - This action opens the decision window itself on success, so it does not require lotat-node-enter.cs after it.
     */
    public bool Execute()
    {
        const string logPrefix = "LotAT Dice Roll";
        ...
        try
        {
            ...
            string roller = NormalizeUser(GetSenderUser());
            ...
            {
                CPH.LogError($"[{logPrefix}] Runtime story load failed while processing !roll. {loadFailure}");
                return true;
            }
            ...
            if (node == null)
            {
                CPH.LogError($"[{logPrefix}] Current node could not be resolved while processing !roll. nodeId='{currentNodeId}', path='{resolvedPath}'.");
                return true;
            }

            if (node.DiceHook == null || !node.DiceHook.Enabled)
            {
                CPH.LogWarn($"[{logPrefix}] !roll ignored because the current node no longer has an active dice_hook payload. nodeId='{currentNodeId}'.");
                return true;
            }
            ...
            {
                CPH.LogWarn($"[{logPrefix}] !roll ignored because runtime dice threshold state no longer matches the authored node. nodeId='{currentNodeId}', runtimeThreshold={thresholdFromState}, authoredThreshold={authoredThreshold}.");
                return true;
            }
            ...
            lock (RNG)
                roll = RNG.Next(1, 101);
            bool success = roll >= authoredThreshold;

            CPH.SendMessage($"🎲 {roller} rolls {roll} (need {authoredThreshold}+).");

            string sessionId = CPH.GetGlobalVar<string>(VAR_LOTAT_SESSION_ID, false) ?? "";
            CPH.LogWarn($"[{logPrefix}] Dice roll recorded. sessionId='{sessionId}', nodeId='{currentNodeId}', user='{roller}', roll={roll}, threshold={authoredThreshold}, success={success}.");
            ...
            {
                CPH.LogError($"[{logPrefix}] Could not open decision window after dice success. {commandFailure}");
                return true;
            }
        ...
        {
            string sessionId = CPH.GetGlobalVar<string>(VAR_LOTAT_SESSION_ID, false) ?? "";
            CPH.LogError($"[{logPrefix}] Unhandled exception while processing !roll. sessionId='{sessionId}', exception={ex}");
            return true;
        }
    }

    private bool IsDiceWindowOpen()
    {
    ...
    private void OpenDecisionWindow(string logPrefix, string currentNodeId, List<string> allowedDecisionCommands, RuntimeStoryNode node)
    {
        ...
        string sessionId = CPH.GetGlobalVar<string>(VAR_LOTAT_SESSION_ID, false) ?? "";
        CPH.LogWarn($"[{logPrefix}] Dice window handed off to the normal decision window. sessionId='{sessionId}', nodeId='{currentNodeId}', seconds={DECISION_WINDOW_SECONDS}, allowedCommands='{string.Join(",", allowedDecisionCommands)}'.");
    }
    ...
    private bool TryLoadRuntimeStory(string logPrefix, out RuntimeStoryDefinition story, out string resolvedPath, out string failureReason)
    {
        ...
        CPH.LogWarn($"[{logPrefix}] Runtime story loaded for dice-roll handling. storyId='{story.StoryId}', path='{resolvedPath}'.");
        return true;
    }

// ACTION-CONTRACT: Actions/Squad/contracts.md#Clone/clone-empire-tick.cs
...
public class CPHInline
{
    ...
    /*
     * Purpose:
     * - Runs the repeating Clone Empire game tick during the movement phase.
     * - Checks for win, kills inactive players, applies Rule 3 collapse to newly
     *   added empire cells, and ends the game on win or loss.
     *
     * Expected trigger/input:
     * - Streamer.bot timer trigger for Clone - Game Tick.
     * - No args required.
     *
     * Required runtime variables:
     * - Reads/writes minigame_active, minigame_name.
     ...
     * - Ends the game when rebels survive 5 minutes.
     * - Kills players who have been idle longer than 30 seconds.
     * - Adds empire cells at inactivity death positions and runs Rule 3 on them.
     * - Publishes squad.clone.update and squad.clone.end broker messages.
     * - Triggers the Mix It Up Clone unlock command on win.
     *
     * Operator notes:
     * - Wire this script to the repeating timer named Clone - Game Tick.
     * - The timer should be set to a 5-second repeating interval in Streamer.bot.
     * - Replace the Mix It Up unlock command ID placeholder before production use.
     */
    public bool Execute()
    {
        ...
        long gameStartUtc = CPH.GetGlobalVar<long?>(VAR_EMPIRE_GAME_START_UTC, false) ?? 0L;
        if (gameStartUtc <= 0)
            return true;
        ...
        if (now - gameStartUtc >= EMPIRE_WIN_DURATION_MS && players.Count > 0)
        {
            TriggerWin(players, empire);
            return true;
        }
        ...
        for (int i = 0; i < players.Count; i++)
        {
            ...
            if (now - player.LastMoveUtc > EMPIRE_INACTIVITY_KILL_MS)
            {
                killedPlayers.Add(player);
                if (AddEmpireCellIfMissing(empire, player.Col, player.Row))
                {
        ...
        {
            EmpirePlayer dead = killedPlayers[i];
            players.RemoveAll(p => string.Equals(p.UserId ?? "", dead.UserId ?? "", StringComparison.OrdinalIgnoreCase));
        }
        ...
        if (stateChanged)
            PublishCloneUpdate("player_inactivity", players, empire, BuildInactivityDetail(killedPlayers));

        long elapsedSeconds = (now - gameStartUtc) / 1000L;
        if (elapsedSeconds > 0 && elapsedSeconds % 60L == 0)
        {
            long remaining = (EMPIRE_WIN_DURATION_MS - (now - gameStartUtc)) / 1000L;
            if (remaining < 0)
                remaining = 0;
    ...
    private string BuildInactivityDetail(List<EmpirePlayer> killedPlayers)
    {
        ...
        for (int i = 0; i < killedPlayers.Count; i++)
            names.Add(killedPlayers[i].UserName ?? "");
    ...
    private List<EmpirePlayer> LoadPlayers()
    {
        ...
        {
            CPH.LogWarn($"[Clone Empire Tick] Failed to parse player JSON. {ex.Message}");
    ...
    private List<EmpireCell> LoadEmpire()
    {
        ...
        {
            CPH.LogWarn($"[Clone Empire Tick] Failed to parse empire JSON. {ex.Message}");
    ...
    private void TriggerWin(List<EmpirePlayer> survivors, List<EmpireCell> empire)
    {
        ...
        string names = string.Join(", ", survivors.ConvertAll(p => p.UserName).ToArray());
    ...
    private void PublishBrokerMessage(string topic, string payloadJson)
    {
        ...
        {
            CPH.LogError($"[Clone Empire Tick] Failed to publish broker topic '{topic}': {ex}");
        }
    }

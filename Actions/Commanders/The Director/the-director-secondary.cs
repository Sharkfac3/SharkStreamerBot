...
public class CPHInline
{
     ...
     * - Streamer.bot action trigger for !secondary command.
     ...
     * - Replace MIXITUP_COMMAND_ID_SECONDARY placeholder when ready.
     */

    private const string LOG_PREFIX = "The Director: Secondary";

    private const string VAR_CURRENT_THE_DIRECTOR = "current_the_director";

    // Replace with the Mix It Up command ID for the secondary switch effect when ready.
    private const string MIXITUP_API_BASE_URL         = "http://localhost:8911";
    private const string MIXITUP_COMMAND_ID_SECONDARY = "REPLACE_WITH_SECONDARY_COMMAND_ID";
    ...
    // Keep this map in sync with the-director-primary.cs.
    ...
    public bool Execute()
    {
        ...
        {
            CPH.LogWarn($"[{LOG_PREFIX}] No active Director. Ignoring !secondary from '{caller}'.");
            return true;
        }
        ...
        CPH.ObsShowSource(currentScene, sources.Secondary);
        CPH.ObsHideSource(currentScene, sources.Primary);

        CPH.LogWarn($"[{LOG_PREFIX}] '{currentScene}' -> secondary. Show: '{sources.Secondary}' | Hide: '{sources.Primary}'.");

        TriggerMixItUpCommand(MIXITUP_COMMAND_ID_SECONDARY);
    ...
    private bool TriggerMixItUpCommand(string commandId)
    {
        ...
        try
        {
            ...
            string payload = JsonSerializer.Serialize(new
            {
                Platform           = "Twitch",
                Arguments          = "secondary",
                SpecialIdentifiers = new { state = "secondary" },

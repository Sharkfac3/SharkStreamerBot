# Prompt: Thin comments in `Actions/Twitch Core Integrations/subscription-counter-rollover.cs`
...
Task:
Thin the comments in `Actions/Twitch Core Integrations/subscription-counter-rollover.cs` so the script does not duplicate behavior already described in `Actions/Twitch Core Integrations/AGENTS.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Twitch Core Integrations/subscription-counter-rollover.cs` unless you discover the behavior reference is missing or materially wrong.
...
- List paste target: `Actions/Twitch Core Integrations/subscription-counter-rollover.cs`.

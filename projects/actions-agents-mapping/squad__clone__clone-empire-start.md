# Prompt: Thin comments in `Actions/Squad/Clone/clone-empire-start.cs`
...
- `Actions/Squad/Clone/README.md` (local script behavior reference)

Task:
Thin the comments in `Actions/Squad/Clone/clone-empire-start.cs` so the script does not duplicate behavior already described in `Actions/Squad/Clone/README.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Squad/Clone/clone-empire-start.cs` unless you discover the behavior reference is missing or materially wrong.
4. If the behavior reference is wrong, update `Actions/Squad/Clone/README.md` first. If the script uses a machine-readable action contract in an `AGENTS.md`, refresh the action-contract stamp after contract changes.
...
- List paste target: `Actions/Squad/Clone/clone-empire-start.cs`.

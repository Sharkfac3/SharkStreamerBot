# Prompt: Thin comments in `Actions/Rest Focus Loop/rest-focus-loop-start.cs`
...
Task:
Thin the comments in `Actions/Rest Focus Loop/rest-focus-loop-start.cs` so the script does not duplicate behavior already described in `Actions/Rest Focus Loop/AGENTS.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Rest Focus Loop/rest-focus-loop-start.cs` unless you discover the behavior reference is missing or materially wrong.
...
- List paste target: `Actions/Rest Focus Loop/rest-focus-loop-start.cs`.

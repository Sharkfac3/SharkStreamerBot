# Prompt: Thin comments in `Actions/Squad/Clone/clone-empire-tick.cs`
...
Task:
Thin the comments in `Actions/Squad/Clone/clone-empire-tick.cs` so the script does not duplicate behavior already described in `Actions/Squad/Clone/README.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Squad/Clone/clone-empire-tick.cs` unless you discover the behavior reference is missing or materially wrong.
...
- List paste target: `Actions/Squad/Clone/clone-empire-tick.cs`.

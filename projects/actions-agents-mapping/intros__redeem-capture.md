# Prompt: Thin comments in `Actions/Intros/redeem-capture.cs`
...
Task:
Thin the comments in `Actions/Intros/redeem-capture.cs` so the script does not duplicate behavior already described in `Actions/Intros/AGENTS.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Intros/redeem-capture.cs` unless you discover the behavior reference is missing or materially wrong.
...
- List paste target: `Actions/Intros/redeem-capture.cs`.

# Prompt: Thin comments in `Actions/Destroyer/destroyer-move.cs`
...
Task:
Thin the comments in `Actions/Destroyer/destroyer-move.cs` so the script does not duplicate behavior already described in `Actions/Destroyer/AGENTS.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Destroyer/destroyer-move.cs` unless you discover the behavior reference is missing or materially wrong.
...
- List paste target: `Actions/Destroyer/destroyer-move.cs`.

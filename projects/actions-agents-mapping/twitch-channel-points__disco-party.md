# Prompt: Thin comments in `Actions/Twitch Channel Points/disco-party.cs`
...
Task:
Thin the comments in `Actions/Twitch Channel Points/disco-party.cs` so the script does not duplicate behavior already described in `Actions/Twitch Channel Points/AGENTS.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Twitch Channel Points/disco-party.cs` unless you discover the behavior reference is missing or materially wrong.
...
- List paste target: `Actions/Twitch Channel Points/disco-party.cs`.

# Prompt: Thin comments in `Actions/Commanders/Captain Stretch/captain-stretch-generalfocus.cs`
...
- `Actions/Commanders/Captain Stretch/README.md` (local script behavior reference)

Task:
Thin the comments in `Actions/Commanders/Captain Stretch/captain-stretch-generalfocus.cs` so the script does not duplicate behavior already described in `Actions/Commanders/Captain Stretch/README.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Commanders/Captain Stretch/captain-stretch-generalfocus.cs` unless you discover the behavior reference is missing or materially wrong.
4. If the behavior reference is wrong, update `Actions/Commanders/Captain Stretch/README.md` first. If the script uses a machine-readable action contract in an `AGENTS.md`, refresh the action-contract stamp after contract changes.
...
- List paste target: `Actions/Commanders/Captain Stretch/captain-stretch-generalfocus.cs`.

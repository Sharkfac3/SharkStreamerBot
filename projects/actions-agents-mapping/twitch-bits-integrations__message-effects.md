# Prompt: Thin comments in `Actions/Twitch Bits Integrations/message-effects.cs`
...
Task:
Thin the comments in `Actions/Twitch Bits Integrations/message-effects.cs` so the script does not duplicate behavior already described in `Actions/Twitch Bits Integrations/AGENTS.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Twitch Bits Integrations/message-effects.cs` unless you discover the behavior reference is missing or materially wrong.
...
- List paste target: `Actions/Twitch Bits Integrations/message-effects.cs`.

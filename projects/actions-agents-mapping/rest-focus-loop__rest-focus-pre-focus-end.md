# Prompt: Thin comments in `Actions/Rest Focus Loop/rest-focus-pre-focus-end.cs`
...
- `Actions/Rest Focus Loop/AGENTS.md` (local action contract/source-of-truth)

Task:
Thin the comments in `Actions/Rest Focus Loop/rest-focus-pre-focus-end.cs` so the script does not duplicate behavior already described in `Actions/Rest Focus Loop/AGENTS.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Rest Focus Loop/rest-focus-pre-focus-end.cs` unless you discover the behavior reference is missing or materially wrong.
4. If the behavior reference is wrong, update `Actions/Rest Focus Loop/AGENTS.md` first. If the script uses a machine-readable action contract in an `AGENTS.md`, refresh the action-contract stamp after contract changes.
...
7. Replace large explanatory header/section comments with brief references such as:
   - `// Runtime source of truth: Actions/Rest Focus Loop/AGENTS.md`
...
- List paste target: `Actions/Rest Focus Loop/rest-focus-pre-focus-end.cs`.

# Prompt: Thin comments in `Actions/Squad/Toothless/overlay-publish.cs`
...
- `Actions/Squad/Toothless/README.md` (local script behavior reference)

Task:
Thin the comments in `Actions/Squad/Toothless/overlay-publish.cs` so the script does not duplicate behavior already described in `Actions/Squad/Toothless/README.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Squad/Toothless/overlay-publish.cs` unless you discover the behavior reference is missing or materially wrong.
4. If the behavior reference is wrong, update `Actions/Squad/Toothless/README.md` first. If the script uses a machine-readable action contract in an `AGENTS.md`, refresh the action-contract stamp after contract changes.
...
7. Replace large explanatory header/section comments with brief references such as:
   - `// Runtime source of truth: Actions/Squad/Toothless/README.md`
...
- List paste target: `Actions/Squad/Toothless/overlay-publish.cs`.

# Prompt: Thin comments in `Actions/Commanders/Water Wizard/water-wizard-redeem.cs`
...
Task:
Thin the comments in `Actions/Commanders/Water Wizard/water-wizard-redeem.cs` so the script does not duplicate behavior already described in `Actions/Commanders/Water Wizard/README.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Commanders/Water Wizard/water-wizard-redeem.cs` unless you discover the behavior reference is missing or materially wrong.
...
- List paste target: `Actions/Commanders/Water Wizard/water-wizard-redeem.cs`.

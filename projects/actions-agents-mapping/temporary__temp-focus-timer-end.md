# Prompt: Thin comments in `Actions/Temporary/temp-focus-timer-end.cs`
...
Task:
Thin the comments in `Actions/Temporary/temp-focus-timer-end.cs` so the script does not duplicate behavior already described in `Actions/Temporary/AGENTS.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Temporary/temp-focus-timer-end.cs` unless you discover the behavior reference is missing or materially wrong.
...
- List paste target: `Actions/Temporary/temp-focus-timer-end.cs`.

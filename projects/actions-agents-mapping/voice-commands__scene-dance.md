# Prompt: Thin comments in `Actions/Voice Commands/scene-dance.cs`

You are working in the SharkStreamerBot repo as `streamerbot-dev`.

Target script:
- `Actions/Voice Commands/scene-dance.cs`
...
Task:
Thin the comments in `Actions/Voice Commands/scene-dance.cs` so the script does not duplicate behavior already described in `Actions/Voice Commands/AGENTS.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Voice Commands/scene-dance.cs` unless you discover the behavior reference is missing or materially wrong.
...
- List paste target: `Actions/Voice Commands/scene-dance.cs`.

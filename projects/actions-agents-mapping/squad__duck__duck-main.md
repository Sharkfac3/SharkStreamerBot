# Prompt: Thin comments in `Actions/Squad/Duck/duck-main.cs`
...
Task:
Thin the comments in `Actions/Squad/Duck/duck-main.cs` so the script does not duplicate behavior already described in `Actions/Squad/Duck/README.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Squad/Duck/duck-main.cs` unless you discover the behavior reference is missing or materially wrong.
...
- List paste target: `Actions/Squad/Duck/duck-main.cs`.

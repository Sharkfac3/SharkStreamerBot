# Prompt: Thin comments in `Actions/Twitch Hype Train/hype-train-start.cs`
...
Task:
Thin the comments in `Actions/Twitch Hype Train/hype-train-start.cs` so the script does not duplicate behavior already described in `Actions/Twitch Hype Train/AGENTS.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Twitch Hype Train/hype-train-start.cs` unless you discover the behavior reference is missing or materially wrong.
...
- List paste target: `Actions/Twitch Hype Train/hype-train-start.cs`.

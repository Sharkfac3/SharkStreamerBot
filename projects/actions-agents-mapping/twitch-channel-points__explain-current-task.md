# Prompt: Thin comments in `Actions/Twitch Channel Points/explain-current-task.cs`

You are working in the SharkStreamerBot repo as `streamerbot-dev`.

Target script:
- `Actions/Twitch Channel Points/explain-current-task.cs`
...
- `Actions/Twitch Channel Points/AGENTS.md` (local action contract/source-of-truth)

Task:
Thin the comments in `Actions/Twitch Channel Points/explain-current-task.cs` so the script does not duplicate behavior already described in `Actions/Twitch Channel Points/AGENTS.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Twitch Channel Points/explain-current-task.cs` unless you discover the behavior reference is missing or materially wrong.
4. If the behavior reference is wrong, update `Actions/Twitch Channel Points/AGENTS.md` first. If the script uses a machine-readable action contract in an `AGENTS.md`, refresh the action-contract stamp after contract changes.
...
7. Replace large explanatory header/section comments with brief references such as:
   - `// Runtime source of truth: Actions/Twitch Channel Points/AGENTS.md`
...
- List paste target: `Actions/Twitch Channel Points/explain-current-task.cs`.

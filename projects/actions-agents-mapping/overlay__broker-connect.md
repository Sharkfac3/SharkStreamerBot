# Prompt: Thin comments in `Actions/Overlay/broker-connect.cs`
...
Task:
Thin the comments in `Actions/Overlay/broker-connect.cs` so the script does not duplicate behavior already described in `Actions/Overlay/AGENTS.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/Overlay/broker-connect.cs` unless you discover the behavior reference is missing or materially wrong.
...
- List paste target: `Actions/Overlay/broker-connect.cs`.

# Prompt: Thin comments in `Actions/LotAT/lotat-join.cs`
...
Task:
Thin the comments in `Actions/LotAT/lotat-join.cs` so the script does not duplicate behavior already described in `Actions/LotAT/AGENTS.md`. The behavior docs are the source of truth; the script should reference them instead of restating long narrative explanations.
...
3. Edit only `Actions/LotAT/lotat-join.cs` unless you discover the behavior reference is missing or materially wrong.
...
- List paste target: `Actions/LotAT/lotat-join.cs`.

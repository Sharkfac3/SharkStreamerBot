...
def build_runtime_options(args: argparse.Namespace, settings: Settings) -> dict[str, Any]:
    ...
    return {
        ...
        "feedback_prompt_path": settings.feedback_prompt_path,
    }
...
def build_prompt(window: dict[str, Any], max_highlights: int, feedback_context: str | None = None) -> str:
    ...
    feedback_block = ""
    if feedback_context:
        feedback_block = f"\n\nPerformance feedback from already-published clips:\n{feedback_context.strip()}"
...
- window_end_seconds: {window['end']:.2f}{feedback_block}
...
def load_feedback_context(path: Any) -> str | None:
    """Load optional prompt context generated by feedback.py."""
    if path is None:
        return None
...
def detect_for_transcript(transcript_path: Path, output_file: Path, options: dict[str, Any]) -> tuple[int, int]:
    ...
    feedback_context = load_feedback_context(options.get("feedback_prompt_path"))
    ...
    for window in windows:
        prompt = build_prompt(
            window,
            int(options["max_highlights_per_window"]),

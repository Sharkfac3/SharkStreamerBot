...
- Streamer.bot action scripts: run script sanity checks and plan Streamer.bot smoke tests.
...
5. For Streamer.bot scripts, confirm syntax sanity and compare any globals, timers, OBS names, and persistent state against Actions shared constants.
...
Validation has two layers: repository-side sanity and Streamer.bot-side smoke testing after paste. Do not claim runtime validation passed if only repository-side checks ran.
